...
// Reads the worker workbook that the team fills in on Google Sheets, and writes
// the matching template. The layout is documented in scripts/make-sheet-template.py;
...
// Checked in order, so "kierownik domyslny" is never swallowed by "kierownik".
const HEADER_FALLBACK_ORDER = ['defaultManager', 'managerQualified', 'name', 'target', 'preference', 'pair24', 'categories'];

const PERIOD_VALUES: Record<WorkerPreference, string[]> = {
...
const AVAILABILITY_CODES: Record<AvailabilityPeriod, string[]> = {
...
const TRUE_VALUES = ['tak', 't', 'yes', 'y', 'true', 'prawda', 'x', '1'];
const FALSE_VALUES = ['nie', 'n', 'no', 'false', 'falsz', 'brak', '0'];
const NO_PAIR_VALUES = ['nie', 'brak', 'zadne', 'none', 'no', 'false'];
const ANY_PAIR_VALUES = ['dowolny', 'dowolne', 'dowolna', 'kazdy', 'obojetnie', 'any', 'either', 'all'];
const POLISH_MONTHS = ['stycz', 'luty|lut', 'marz|marc', 'kwiec', 'maj', 'czerw', 'lip', 'sierp', 'wrzes|wrześ', 'pazdzier', 'listopad', 'grudz'];
...
// Guards against a note or instruction line being read as somebody's name.
...
// Google Sheets exports values, but a raw template read can still carry a formula string.
...
function cellRows(table: SheetTable): unknown[][] {
  return (table.rows || []).map(row => (Array.isArray(row) ? row : []));
}
function rowIsEmpty(row: unknown[]): boolean {
  return !row.some(cell => cellText(cell) !== '');
}
function matchesSynonym(header: string, field: string): boolean {
  const value = normalise(header);
  if (!value) return false;
  return HEADER_SYNONYMS[field].includes(value);
}
// Exact only: used on data cells too, where "Nieznana Osoba" must stay a person.
function isHeaderLabel(value: string): string | null {
  return HEADER_FALLBACK_ORDER.find(field => matchesSynonym(value, field)) || null;
}
function headerField(header: string): string | null {
  const value = normalise(header);
...
function normaliseMonth(value: unknown): string | null {
  ...
  if (iso) { const month = String(Number(iso[2])).padStart(2, '0'); return isMonth(iso[1] + '-' + month) ? iso[1] + '-' + month : null; }
...
function findMonth(rows: unknown[][]): string | null {
  for (const row of rows.slice(0, 8)) for (const cell of row) { const month = normaliseMonth(cell); if (month) return month; }
  return null;
}
...
interface ParsedAvailabilityRow {
  sheet: string; row: number; name: string; workerId: string | null; isNew: boolean;
  entries: { date: string; period: AvailabilityPeriod }[];
}
interface SheetImportResult {
  month: string; monthFromSheet: boolean;
  workers: ParsedWorkerRow[]; availability: ParsedAvailabilityRow[]; issues: SheetIssue[];
  newCategories: string[]; sheetsUsed: { name: string; kind: string }[];
}
interface SheetImportContext { workers: ScheduleWorker[]; pairings: Pairing[]; categories: string[]; currentMonth: string; }
interface SheetImportOptions { addNewWorkers?: boolean; updateWorkers?: boolean; replaceAvailability?: boolean; makeId?: () => string; }
interface SheetImportBackup { workers: ScheduleWorker[]; availability: AvailabilityEntry[]; categories: string[]; month: string; at: string; }

function classifySheet(table: SheetTable): 'workers' | 'availability' | 'ignored' | null {
...
function findHeaderRow(rows: unknown[][], predicate: (row: unknown[]) => boolean): number {
  for (let index = 0; index < Math.min(rows.length, 12); index++) if (predicate(rows[index])) return index;
  return -1;
}
function matchWorker(name: string, context: SheetImportContext): { worker: ScheduleWorker | null; matchedBy: 'exact' | 'reordered' | null } {
...
function parsePreference(value: string): WorkerPreference | null {
  const text = normalise(value);
  if (!text) return null;
  return (Object.keys(PERIOD_VALUES) as WorkerPreference[]).find(period => PERIOD_VALUES[period].includes(text)) || null;
}
function parsePair24(value: string, pairings: Pairing[]): string | null {
  const text = normalise(value);
  if (!text) return null;
  ...
  // "20-20" and "20:00 → 20:00" describe the same pairing.
...
// "08:00 → 08:00" and "8-8" both reduce to "8-8"; minutes are dropped.
function hourSignature(value: string): string {
...
function parseCategories(value: string, known: string[]): string[] {
  const parts = value.split(/[,;/|]+/).map(part => part.trim()).filter(Boolean);
  return parts.map(part => known.find(category => normalise(category) === normalise(part)) || part);
}
function parseAvailabilityCode(value: string): AvailabilityPeriod | null {
  const text = normalise(value);
  if (!text) return null;
  return (Object.keys(AVAILABILITY_CODES) as AvailabilityPeriod[]).find(period => AVAILABILITY_CODES[period].includes(text)) || null;
}
function describeValue(field: string, value: unknown, context: SheetImportContext): string {
  ...
  if (field === 'categories') return (value as string[]).join(', ');
...
function parseWorkersTable(table: SheetTable, context: SheetImportContext, result: SheetImportResult): void {
  ...
  rows[headerIndex].forEach((cell, index) => { const field = headerField(cellText(cell)); if (field && columns[field] === undefined) columns[field] = index; });
  ...
  for (let index = headerIndex + 1; index < rows.length; index++) {
    ...
    if (!name) { result.issues.push({ level: 'warning', sheet: table.name, row: rowNumber, message: 'Row has data but no name, so it was skipped.' }); continue; }
    if (isHeaderLabel(name)) continue;
    if (looksLikeNote(name)) { result.issues.push({ level: 'warning', sheet: table.name, row: rowNumber, message: 'The name column reads like a note ("' + name.slice(0, 40) + '…"), so the row was skipped.' }); continue; }
    const key = nameKey(name);
    if (seen.has(key)) { result.issues.push({ level: 'warning', sheet: table.name, row: rowNumber, message: '"' + name + '" also appears in row ' + seen.get(key) + '. Only the first row was used.' }); continue; }
    seen.set(key, rowNumber);
    const values: Partial<ScheduleWorker> = {};
    ...
    if (columns.preference !== undefined) {
    ...
    if (columns.categories !== undefined) {
    ...
    if (columns.managerQualified !== undefined) { const value = parseBoolean(cellText(row[columns.managerQualified])); if (value !== null) values.managerQualified = value; }
    if (columns.defaultManager !== undefined) {
...
function parseAvailabilityTable(table: SheetTable, context: SheetImportContext, result: SheetImportResult): void {
  ...
  if (month) { result.month = month; result.monthFromSheet = true; }
  ...
  if (headerIndex < 0) { result.issues.push({ level: 'error', sheet: table.name, message: 'No header row with a name column and day numbers was found, so this tab was skipped.' }); return; }
  ...
  rows[headerIndex].forEach((cell, index) => { const day = dayNumber(cell); if (day !== null && index !== nameColumn) dayColumns.push({ column: index, day }); });
  ...
  const seen = new Map<string, ParsedAvailabilityRow>();
  for (let index = headerIndex + 1; index < rows.length; index++) {
    const row = rows[index], rowNumber = index + 1;
    if (rowIsEmpty(row)) continue;
    const name = cellText(row[nameColumn]);
    if (!name) { result.issues.push({ level: 'warning', sheet: table.name, row: rowNumber, message: 'Availability was filled in without a name, so the row was skipped.' }); continue; }
    if (isHeaderLabel(name) || looksLikeNote(name)) continue;
    const match = matchWorker(name, context);
    const existing = seen.get(nameKey(name));
    const parsed: ParsedAvailabilityRow = existing || { sheet: table.name, row: rowNumber, name, workerId: match.worker?.id || null, isNew: !match.worker, entries: [] };
    ...
    for (const entry of dayColumns) {
      ...
      if (!period) { result.issues.push({ level: 'warning', sheet: table.name, row: rowNumber, message: 'Unknown code "' + raw + '" on day ' + entry.day + ' for ' + name + '. Use X, D or N.' }); continue; }
    ...
    if (!existing) { seen.set(nameKey(name), parsed); result.availability.push(parsed); }
  }
}

function parseSheets(tables: SheetTable[], context: SheetImportContext): SheetImportResult {
  const result: SheetImportResult = { month: context.currentMonth, monthFromSheet: false, workers: [], availability: [], issues: [], newCategories: [], sheetsUsed: [] };
  const classified = tables.map(table => ({ table, kind: classifySheet(table) }));
  // Workers first: availability rows are matched against the names they introduce.
  ...
  result.availability.filter(row => row.isNew && !result.workers.some(worker => nameKey(worker.name) === nameKey(row.name)))
    .forEach(row => result.issues.push({ level: 'warning', sheet: row.sheet, row: row.row, message: '"' + row.name + '" is not on the Pracownicy tab and does not match anyone in the app, so their availability needs a worker first.' }));
  return result;
}

function importSummary(result: SheetImportResult, options: SheetImportOptions = {}): { workersUpdated: number; workersNew: number; availabilityWorkers: number; availabilityDays: number; blocked: number } {
  const addNew = options.addNewWorkers !== false;
  const known = (row: ParsedAvailabilityRow) => Boolean(row.workerId) || (addNew && result.workers.some(worker => worker.isNew && nameKey(worker.name) === nameKey(row.name)));
...
function applyImport(app: AppState, result: SheetImportResult, options: SheetImportOptions = {}): SheetImportBackup {
  ...
  const backup: SheetImportBackup = { workers: JSON.parse(JSON.stringify(app.workers)), availability: JSON.parse(JSON.stringify(app.availability)), categories: [...app.categories], month: result.month, at: new Date().toISOString() };
  result.newCategories.forEach(category => { if (!app.categories.some(known => normalise(known) === normalise(category))) app.categories.push(category); });
  ...
  result.workers.forEach(row => {
    if (row.isNew) {
      ...
      const worker: ScheduleWorker = { id: makeId(), name: row.name, target: DEFAULT_TARGET_HOURS, preference: 'either', pair24: 'none', categories: ['General'], managerQualified: false, defaultManager: false, ...row.values };
      app.workers.push(worker);
      idByName.set(nameKey(worker.name), worker.id);
      return;
    }
  ...
  if (promoted) { const id = idByName.get(nameKey(promoted.name)); app.workers.forEach(worker => { if (worker.defaultManager && worker.id !== id) worker.defaultManager = false; }); }
...
function restoreImportBackup(app: AppState, backup: SheetImportBackup): void {
...
// Mirrors scripts/make-sheet-template.py so an exported template imports cleanly.
function buildTemplateTables(app: AppState, month: string): SheetTable[] {
  const total = daysInMonth(month), [year, index] = month.split('-').map(Number);
  const weekdays = ['Pn', 'Wt', 'Śr', 'Cz', 'Pt', 'So', 'Nd'];
  const pairName = (worker: ScheduleWorker) => worker.pair24 === 'any' ? 'Dowolny' : (app.pairings.find(pairing => pairing.id === worker.pair24)?.name || 'Nie');
  const instructions: unknown[][] = [
    ['Grafik — dane od pracowników'],
    ['Miesiąc: ' + month],
    [],
...
export {
  parseSheets, applyImport, restoreImportBackup, buildTemplateTables, importSummary,
  classifySheet, normalise, nameKey, normaliseMonth, parsePreference, parsePair24, parseAvailabilityCode, parseBoolean, headerField, daysInMonth
};
