...
const DAY_MS = 86400000;
  ...
  function dateAtNoon(year, month, day) { return new Date(year, month, day, 12); }
  function iso(date) { return new Date(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())).toISOString().slice(0, 10); }
  function addDays(value, amount) { const date = new Date(value + 'T12:00:00'); date.setDate(date.getDate() + amount); return iso(date); }
  function daysInMonth(year, month) { return new Date(year, month + 1, 0).getDate(); }
  function startTimestamp(date, time) { return new Date(date + 'T' + time + ':00').getTime(); }
  function intervalFor(date: string, shift: Shift): Interval {
    const start = startTimestamp(date, shift.start);
    let end = startTimestamp(date, shift.end);
    if (end <= start) end = new Date(addDays(date, 1) + 'T' + shift.end + ':00').getTime();
    return { start, end, minutes: Math.round((end - start) / 60000) };
  }
  function gapMinutes(a: Interval, b: Interval): number {
    if (a.end <= b.start) return (b.start - a.end) / 60000;
    if (b.end <= a.start) return (a.start - b.end) / 60000;
    return -1;
  }
  function overlapsOrTooClose(a, b, minimumRestMinutes) { const gap = gapMinutes(a, b); return gap < minimumRestMinutes; }
  ...
  function gcd(a, b) { while (b) { const next = a % b; a = b; b = next; } return Math.abs(a); }
  ...
  function jsonModelToLp(model: any): string {
    ...
    Object.entries(model.constraints as Record<string, { equal?: number; min?: number; max?: number }>).forEach(([constraintName, bounds]) => {
      ...
      if (bounds.equal !== undefined) rows.push(safe + ': ' + expression + ' = ' + bounds.equal);
      else { if (bounds.min !== undefined) rows.push(safe + '_min: ' + expression + ' >= ' + bounds.min); if (bounds.max !== undefined) rows.push(safe + '_max: ' + expression + ' <= ' + bounds.max); }
    });
    return ['Minimize',' obj: '+linearExpression(objective),'Subject To',...rows,'Binary',' '+Object.keys(model.binaries || {}).join(' '),'End'].join('\n');
  }
  function highsAdapter(highs: any): SolverLike {
    return { Solve(model) { const result = highs.solve(jsonModelToLp(model), { presolve: 'on', mip_rel_gap: 0 }); const solution: any = { feasible: result.Status === 'Optimal', result: result.ObjectiveValue, status: result.Status }; Object.entries(result.Columns || {} as Record<string, { Primal: number }>).forEach(([name, column]: [string, any]) => { if (column.Primal) solution[name] = column.Primal; }); return solution; } };
  }

  function buildInstances(config: AppState): ShiftInstance[] {
    const { year, month } = parseMonth(config.current), instances: ShiftInstance[] = [];
    for (let day = 1; day <= daysInMonth(year, month); day++) {
      const date = iso(dateAtNoon(year, month, day)), weekday = new Date(date + 'T12:00:00').getDay();
  ...
  function pairingContinuity(pairing: Pick<Pairing, 'firstShiftId' | 'secondShiftId' | 'dayOffset'>, shifts: Record<string, Shift>) {
    ...
    if (total !== DAY_MS) return { valid: false, reason: 'The pairing lasts ' + Math.round(total / 360000) / 10 + 'h, not 24h.' };
    return { valid: true, reason: 'Continuous 24h duty.', start: first.start, end: second.end };
  }

  function buildPairOccurrences(config: AppState, instances: ShiftInstance[], byKey: Record<string, ShiftInstance>) {
  ...
  function solve(config: AppState & { randomRanks?: Record<string, number> }, solverInstance?: SolverLike) {
    ...
    const occurrences = buildPairOccurrences(config, instances, byKey), model: any = { optimize: 'cost', opType: 'min', constraints: {}, variables: {}, binaries: {} };
    const variableInfo: Record<string, any> = {}, xBySlotWorker: Record<string, string> = {}, candidatesByWorker: Record<string, any[]> = {}, managerCandidateCount: Record<string, string[]> = {}, diagnostics = { lockConflicts: [] as string[], invalidPairings: [] as string[], candidateCounts: {} as Record<string, number>, modelVariables: 0, modelConstraints: 0 };
    ...
    config.pairings.forEach(pairing => { const check = pairingContinuity(pairing, config.shifts); if (!check.valid) diagnostics.invalidPairings.push(pairing.name + ': ' + check.reason); });

    function constraint(name, definition) { model.constraints[name] = definition; return name; }
    function variable(name, coefficients, info) { model.variables[name] = { cost: 0, ...coefficients }; model.binaries[name] = 1; variableInfo[name] = info; return name; }
    function addCoefficient(variableName, constraintName, value) { model.variables[variableName][constraintName] = (model.variables[variableName][constraintName] || 0) + value; }
    function externalConflicts(worker, instance) {
      ...
      return (config.twentyFourPairs || []).some(pair => {
        ...
        return instance.start >= pairEnd && instance.start < pairEnd + recoveryMinutes * 60000;
      });
    }
    ...
    // Exact slot coverage and candidate variables.
    instances.forEach((instance, slotIndex) => {
      ...
      workers.forEach(worker => {
        const name = 'x_' + slotIndex + '_' + safeName(worker.id), random = ((config.randomRanks && config.randomRanks[worker.id]) || 0) / 100000;
      ...
      if (!lockedWorkerId) variable('u_' + slotIndex, { [cover]: 1, cost: 1000000000000 }, { kind: 'unfilled', instance });
    });

    if (diagnostics.lockConflicts.length) return { feasible: false, error: diagnostics.lockConflicts.join(' '), diagnostics };

    // Configurable continuous 24h pair variables.
    const allowedPairKeys = new Set<string>(), pairVariables: any[] = [];
    occurrences.forEach((occurrence, occurrenceIndex) => {
      config.workers.forEach(worker => {
        ...
        addCoefficient(firstX, c1, -1); addCoefficient(secondX, c2, -1); addCoefficient(firstX, c3, 1); addCoefficient(secondX, c3, 1);
        const token = [occurrence.first.key, occurrence.second.key].sort().join('~'); allowedPairKeys.add(worker.id + '|' + token);
        pairVariables.push({ name, occurrence, worker });
      });
    });

    // Exact overlap and minimum-rest constraints for all assignment intervals.
    Object.entries(candidatesByWorker).forEach(([workerId, candidates]) => {
      for (let i = 0; i < candidates.length; i++) for (let j = i + 1; j < candidates.length; j++) {
        ...
        const name = constraint('rest_' + safeName(workerId) + '_' + i + '_' + j, { max: 1 }); addCoefficient(left.name, name, 1); addCoefficient(right.name, name, 1);
      }
    });

    // Recovery after any chosen 24h pairing, including conflicts between pairings.
    pairVariables.forEach((pair, index) => {
      (candidatesByWorker[pair.worker.id] || []).forEach((candidate, candidateIndex) => {
        ...
        if (candidate.instance.start >= pair.occurrence.end && candidate.instance.start < pair.occurrence.end + recoveryMinutes * 60000) {
          const name = constraint('recovery_' + index + '_' + candidateIndex, { max: 1 }); addCoefficient(pair.name, name, 1); addCoefficient(candidate.name, name, 1);
        }
      });
    });
    for (let i = 0; i < pairVariables.length; i++) for (let j = i + 1; j < pairVariables.length; j++) {
      const left = pairVariables[i], right = pairVariables[j]; if (left.worker.id !== right.worker.id) continue;
      const first = left.occurrence.start <= right.occurrence.start ? left : right, second = first === left ? right : left;
      if (second.occurrence.start < first.occurrence.end + recoveryMinutes * 60000) { const name = constraint('pairRecovery_' + i + '_' + j, { max: 1 }); addCoefficient(left.name, name, 1); addCoefficient(right.name, name, 1); }
    }

    // Manager fallback: a 24h worker may cover manager only when no qualified manager is available and another worker covers 08:00–16:00.
    instances.filter(instance => instance.shift.manager).forEach((managerInstance, managerIndex) => {
      ...
      pairVariables.filter(pair => pair.occurrence.start <= managerInstance.start && pair.occurrence.end >= managerInstance.end).forEach((pair, fallbackIndex) => {
        ...
        supportSlots.forEach(slot => config.workers.forEach(worker => { if (worker.id === pair.worker.id) return; const variableName = xBySlotWorker[slot.key + '|' + worker.id]; if (variableName) supportVariables.push(variableName); }));
        ...
        variable(name, { [coverName]: 1, [link]: 1, [support]: 1, cost: 1 }, { kind: 'managerFallback', instance: managerInstance, worker: pair.worker, pair }); addCoefficient(pair.name, link, -1); supportVariables.forEach(variableName => addCoefficient(variableName, support, -1));
      });
    });

    // Exact squared target-error linearisation over every reachable hour total.
    config.workers.forEach((worker, workerIndex) => {
      ...
      durations.forEach(duration => { const next = new Set(reachable); reachable.forEach(total => next.add(total + duration)); reachable = next; });
      [...reachable].sort((a, b) => a - b).forEach((minutes, optionIndex) => {
        ...
        variable(name, { [choice]: 1, [balance]: -minutes, cost: error * error * 1000 }, { kind: 'hours', worker, minutes });
      });
      const managerVars = managerCandidateCount[worker.id] || [];
      if (managerVars.length) {
        const managerChoice = constraint('managerChoice_' + workerIndex, { equal: 1 }), managerBalance = constraint('managerBalance_' + workerIndex, { equal: 0 }); managerVars.forEach(name => addCoefficient(name, managerBalance, 1));
    ...
    diagnostics.modelVariables = Object.keys(model.variables).length; diagnostics.modelConstraints = Object.keys(model.constraints).length;
    ...
    Object.entries(variableInfo).forEach(([name, info]) => {
      ...
      if (info.kind === 'assignment') {
        ...
        info.occurrence.keys.forEach(key => { assignmentReasons[key] = 'Part of ' + info.occurrence.pairing.name + ' 24h duty; exact recovery constraints applied.'; });
      } else if (info.kind === 'managerFallback') {
        assignments[info.instance.key] = info.worker.id; managerFallbacks[info.instance.date] = info.worker.id; assignmentReasons[info.instance.key] = '24h manager fallback; a separate worker covers the full 08:00–16:00 window.';
      }
    });
  ...
  export { solve, buildInstances, pairingContinuity, intervalFor, gapMinutes, addDays, jsonModelToLp, highsAdapter };
